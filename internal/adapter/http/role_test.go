...
func TestRoleAPI(t *testing.T) {
	...
	handler.ServeHTTP(getRec, httptest.NewRequest(http.MethodGet, "/api/v1/roles/"+id, nil))
	if getRec.Code != http.StatusOK {
		t.Fatalf("get status = %d", getRec.Code)
	}

	listRec := httptest.NewRecorder()
	handler.ServeHTTP(listRec, httptest.NewRequest(http.MethodGet, "/api/v1/roles?q=admin", nil))
	if listRec.Code != http.StatusOK {
		t.Fatalf("list status = %d", listRec.Code)
	}

	updateReq := httptest.NewRequest(http.MethodPut, "/api/v1/roles/"+id, strings.NewReader(`{"name":"admin","description":"Administrator","isActive":false}`))
	...
	handler.ServeHTTP(deleteRec, httptest.NewRequest(http.MethodDelete, "/api/v1/roles/"+id, nil))
	...
	if deleted["id"] != id || deleted["deletedAt"] == nil {
		t.Fatalf("unexpected delete body: %s", deleteRec.Body.String())
	}

	missingRec := httptest.NewRecorder()
	handler.ServeHTTP(missingRec, httptest.NewRequest(http.MethodGet, "/api/v1/roles/"+id, nil))
